...
class ContractValidator:
    ...
    def _validate_interaction(
        self, file_name: str, index: int, interaction: Dict
    ) -> bool:
                ...
                for method, details in methods.items():
                    if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"]:
                        if "responses" not in details:
                            print(
                                f"❌ OpenAPI: {method.upper()} {path} - responses مفقود"
                            )
                            return False
    ...
    def _run_schemathesis_tests(self) -> bool:
        ...
        try:
            ...
            subprocess.run(
                ["schemathesis", "--version"], capture_output=True, check=True
            )
        ...
        for path, methods in paths.items():
            for method, details in methods.items():
                if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"]:
                    ...
                    required_params = [
                        p for p in parameters if p.get("required", False)
                    ]
                    ...
                    if "200" not in responses and "201" not in responses:
                        breaking_changes.append(
                            f"{method.upper()} {path}: لا يوجد response ناجح"
                        )
